...
<title>View App Traffic Service Info</title>
...
.custom-btn {
	width: calc(100% - 10px);
	/* Adjust this value to control the size of the buttons */
	margin: 5px;
}
</style>
</head>
		...
		<div class="page-content form-page">
			...
			<div class="bg-dash-dark-2 py-4">
				<div class="container-fluid">
					<h2 class="h5 mb-0">View App Traffic Service Info</h2>
				</div>
			</div>
			<!-- Breadcrumb-->
				...
				<div class="row gy-4">
					...
					<div class="card mb-0">
						<div class="card-header">
							<h3 class="h4 mb-0">List of all App Traffic Service Infos</h3>
						</div>
						<div class="card-body pt-0">
							...
							<div class="table-responsive">
								<table class="table mb-0 table-striped table-hover"
									...
									<tbody>
										<c:forEach items="${appTrafficServiceInfoList}"
											var="appTrafficServiceInfo">
											<tr class="dataRow">
												<th scope="row">${appTrafficServiceInfo.id}</th>
												<td>${appTrafficServiceInfo.servicename}</td>
												<td>${appTrafficServiceInfo.operator}</td>
												<td>${appTrafficServiceInfo.territory}</td>
												<td>
													<form action="duplicateAppTrafficServiceInfo" method="post">
														<button class="btn btn-secondary custom-btn" type="submit"
															name="appTSId" value="${appTrafficServiceInfo.id}">Duplicate</button>
													</form>
												</td>
												<td><form action="editAppTrafficServiceInfo"
														method="post">
														<input type="hidden" name="territoryName"
															value="${appTrafficServiceInfo.territory}">
														<button class="btn btn-primary custom-btn" type="submit"
															name="appId" value="${appTrafficServiceInfo.id}">Edit</button>
													</form></td>
												<td><form action="viewDetails" method="post"
														target="_blank">
														<button class="btn btn-link" type="submit" name="viewId"
															value="${appTrafficServiceInfo.id}">Details</button>
													</form></td>
												<td><form action="excelReportDownload" method="post">
														<button class="btn btn-light custom-btn" type="submit"
															name="excelId" value="${appTrafficServiceInfo.id}">.xlsx</button>
													</form></td>
											</tr>
										</c:forEach>
									</tbody>
