		...
		<div class="page-content form-page">
			...
			<div class="bg-dash-dark-2 py-4">
				<div class="container-fluid">
					<h2 class="h5 mb-0">Edit Operator</h2>
				</div>
			</div>
			<!-- Breadcrumb-->
					...
					<div class="col-lg-12">
						<div class="card">
							...
							<div class="card-body pt-0">
								<form class="form-horizontal" action="updateOperator"
									method="get">
									<div class="row">
										<label class="col-sm-3 form-label">Name</label>
										<div class="col-sm-9">
											<input class="form-control" value="${operator.name}"
												type="text" name="name" placeholder="Enter Name here..."
												required>
										</div>
									</div>
									<input type="hidden" name="id" value="${operator.id}">
									<div class="my-4"></div>
									<input type="hidden" name="addtime" value="${operator.addtime}">
									<div class="row">
										<label class="col-sm-3 form-label">Select Territory ID</label>
										<div class="col-sm-9">
											<select class="form-select mb-3" name="territoryid" required>
												<c:forEach items="${territoryList}" var="territory">
													<option value="${territory.id}"
														<c:if test="${territory.id == operator.territoryid}">selected</c:if>>${territory.id}
									...
									<div class="my-4"></div>
									<div class="row">
										<label class="col-sm-3 form-label">Operator Initials</label>
										<div class="col-sm-9">
											<input class="form-control" value="${operator.initials}"
												type="text" name="initials"
												placeholder="Enter Operator Initials here..." required>
										</div>
									</div>
									<div class="my-4"></div>
									<div class="row">
		...
		$('select').on('change', function() {
			...
			$.ajax({
				...
				success : function(result) {
					...
					$('#addtime').val(jsonResult.addtime);
					$('#edittime').val(jsonResult.edittime);
					$('#territoryid').val(jsonResult.territoryid);
				}
			});
		});
	</script> -->
